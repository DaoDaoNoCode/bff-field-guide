...
## Operator Gotchas (controller-runtime)

These only bite once you're in the `dashboard-operator`. They're the operator equivalent of the shadowing and nil-map traps above — they compile fine and fail at runtime or in review.

### 14. Forgetting `make generate` / `make manifests` After Editing a Type

**The trap:** You add a field to a type in `api/v1alpha1/` (say a new field on `DashboardSpec`), rebuild, and either the CRD rejects your new field or a `DeepCopy`-related nil-pointer shows up at runtime.

```go
// api/v1alpha1/dashboard_types.go
type DashboardSpec struct {
    Gateway  GatewaySpec `json:"gateway"`
    NewField string      `json:"newField,omitempty"`   // You added this...
}
// ...but you never ran `make generate` or `make manifests`.
```

**What happens:** Two generated artifacts are now stale. `zz_generated.deepcopy.go` doesn't know about the new field (regenerated by `make generate`), and the CRD YAML under `config/crd/bases/` doesn't declare it, so the API server silently drops it (regenerated by `make manifests`).

**The fix:** Any time you touch a file in `api/v1alpha1/`, run both:

```bash
cd dashboard-operator
make generate    # Regenerate DeepCopy methods
make manifests   # Regenerate CRD YAML from kubebuilder markers
make sync-chart-crds   # And push the CRD into the Helm chart
```

::: danger The Single Most Common Operator Mistake
Editing a `*_types.go` file and skipping the codegen step is the #1 operator footgun. There's no compile error — the drift only shows up when a field mysteriously doesn't persist or a test panics in `DeepCopy`. Make it muscle memory: **edit a type → run `make generate manifests`.**
:::

### 15. Blocking Inside `Reconcile`

**The trap:** You need to wait for something (a pod to become ready, an external API), so you `time.Sleep` or poll in a loop inside `Reconcile`.

```go
func (r *DashboardReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
    for !deploymentReady(ctx) {     // WRONG: blocks the worker
        time.Sleep(5 * time.Second) // ...and starves every other CR
    }
    return ctrl.Result{}, nil
}
```

**What happens:** Reconcile runs on a bounded worker pool. A blocked worker can't process any other `Dashboard` CR. Sleep long enough and the whole controller appears frozen.

**The fix:** Return and ask to be requeued. The reconcile loop *is* your polling mechanism.

```go
if !deploymentReady(ctx) {
    return ctrl.Result{RequeueAfter: 5 * time.Second}, nil   // Come back in 5s
}
return ctrl.Result{}, nil
```

::: tip Reconcile Is a Render Function
Think of `Reconcile` like a React render: it should be fast, side-effect-light, and idempotent. If you're tempted to `await` something, return `RequeueAfter` instead and let the controller call you back.
:::

### 16. Server-Side Apply Field-Ownership Conflicts

**The trap:** Two writers — your operator and something else (a user with `oc edit`, another controller) — both try to own the same field via Server-Side Apply, and your `Patch` fails with a conflict error.

```go
// If another manager already owns .spec.replicas, this errors:
r.Patch(ctx, deploy, client.Apply, client.FieldOwner("dashboard-operator"))
```

**What happens:** SSA tracks *which manager owns which field*. A conflict means someone else claimed a field you're now trying to set. Blindly force-applying steals ownership and can start a fight (two controllers flipping the field back and forth — see #17).

**The fix:** Own only the fields you actually manage (don't apply a whole object when you only set a few fields), and use `client.ForceOwnership` **deliberately**, not reflexively:

```go
r.Patch(ctx, deploy, client.Apply,
    client.FieldOwner("dashboard-operator"),
    client.ForceOwnership)   // Only when you're sure the operator should win
```

### 17. Requeue Loops (the operator that never settles)

**The trap:** Your reconcile writes something on every pass that then triggers another reconcile — a status timestamp, a randomized value, or an SSA fight (#16). The operator spins forever, hammering the API server.

```go
dashboard.Status.LastChecked = metav1.Now()   // Changes every reconcile...
r.Status().Update(ctx, &dashboard)             // ...which triggers another reconcile
return ctrl.Result{}, nil
```

**What happens:** Reconcile must be **idempotent** — running it twice with the same input should produce the same output and *no* write the second time. Writing a value that changes every run guarantees an infinite loop.

**The fix:** Only write when something actually changed. Compare desired vs observed before updating status, and never put always-changing values (timestamps, randoms) in a spec/field you also diff against.

```go
if dashboard.Status.Phase != desiredPhase {    // Only update when it truly changed
    dashboard.Status.Phase = desiredPhase
    if err := r.Status().Update(ctx, &dashboard); err != nil {
        return ctrl.Result{}, err
    }
}
return ctrl.Result{}, nil                        // Steady state → no write → no requeue
```

See [The Reconciler](/guide/operator/reconciler) for the full idempotency discussion. And note that gotcha #5 (the nil-map panic) shows up in operator code too — the module registry builds several maps that must be `make`'d before you write to them.
